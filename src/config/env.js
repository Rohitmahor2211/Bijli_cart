...
const envSchema = z.object({
  ...
  MONGO_TEST_URI: z.string().default('mongodb://127.0.0.1:27017/retailer_ecommerce_test'),

  JWT_ACCESS_SECRET: z.string().default('super_secret_access_key_retailer_2026_change_in_prod'),
  JWT_REFRESH_SECRET: z.string().default('super_secret_refresh_key_retailer_2026_change_in_prod'),
  ...
  OTP_EXPIRY_MINUTES: z.string().default('10').transform((val) => parseInt(val, 10)),
  OTP_MAX_ATTEMPTS: z.string().default('3').transform((val) => parseInt(val, 10)),
  ...
  PLATFORM_COMMISSION_PERCENT: z.string().default('10').transform((val) => Number(val)),
  SETTLEMENT_HOLD_DAYS: z.string().default('3').transform((val) => parseInt(val, 10)),
  AUTO_PAYOUT_ENABLED: z.enum(['true', 'false']).default('false').transform((val) => val === 'true'),
  RAZORPAYX_ACCOUNT_NUMBER: z.string().optional().default(''),
  BANK_DATA_ENCRYPTION_KEY: z.string().default('0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef'),
...
if (configuredEnv.AUTO_PAYOUT_ENABLED && configuredEnv.PAYMENT_PROVIDER === 'razorpay' &&
  (!configuredEnv.RAZORPAYX_ACCOUNT_NUMBER || !configuredEnv.RAZORPAYX_WEBHOOK_SECRET)) {
  throw new Error('Automatic RazorpayX payouts require RAZORPAYX_ACCOUNT_NUMBER and RAZORPAYX_WEBHOOK_SECRET.');
}


if (configuredEnv.NODE_ENV === 'production') {
  const requiredProductionValues = ['MONGO_URI', 'CLIENT_URL', 'JWT_ACCESS_SECRET', 'JWT_REFRESH_SECRET'];
  const missingValues = requiredProductionValues.filter((key) => {
    const value = process.env[key];
    return !value || value.startsWith('replace_') || value.includes('change_in_prod');
  });
  ...
  if (configuredEnv.SMS_DELIVERY_ENABLED && configuredEnv.SMS_PROVIDER === 'fast2sms' && (
    ...
    throw new Error('Fast2SMS production configuration is incomplete. Set SMS_API_KEY.');
  }

  if (configuredEnv.PAYMENT_PROVIDER === 'razorpay' && (!configuredEnv.RAZORPAY_KEY_ID || !configuredEnv.RAZORPAY_KEY_SECRET || !configuredEnv.RAZORPAY_WEBHOOK_SECRET)) {
    throw new Error('Razorpay production configuration is incomplete. Set RAZORPAY_KEY_ID, RAZORPAY_KEY_SECRET, and RAZORPAY_WEBHOOK_SECRET.');
  }


  if (configuredEnv.PAYMENT_PROVIDER === 'razorpay' && configuredEnv.AUTO_PAYOUT_ENABLED && !configuredEnv.RAZORPAYX_ACCOUNT_NUMBER) {
    throw new Error('Automatic RazorpayX payouts require RAZORPAYX_ACCOUNT_NUMBER.');
  }

  if (configuredEnv.PAYMENT_PROVIDER === 'razorpay' && configuredEnv.AUTO_PAYOUT_ENABLED && !configuredEnv.RAZORPAYX_WEBHOOK_SECRET) {
    throw new Error('Automatic RazorpayX payouts require RAZORPAYX_WEBHOOK_SECRET.');
  }

  if (configuredEnv.SHIPPING_PROVIDER === 'shiprocket' && (!configuredEnv.SHIPROCKET_EMAIL || !configuredEnv.SHIPROCKET_PASSWORD)) {
    throw new Error('Shiprocket shipping requires SHIPROCKET_EMAIL and SHIPROCKET_PASSWORD.');
  }

  if (configuredEnv.CLOUDINARY_CLOUD_NAME === 'demo_cloud' || !configuredEnv.CLOUDINARY_API_KEY || !configuredEnv.CLOUDINARY_API_SECRET) {
    throw new Error('Production uploads require a configured Cloudinary account.');
  }

  if (!process.env.BANK_DATA_ENCRYPTION_KEY || process.env.BANK_DATA_ENCRYPTION_KEY === '0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef' || process.env.BANK_DATA_ENCRYPTION_KEY.startsWith('replace_')) {
    throw new Error('Set a unique BANK_DATA_ENCRYPTION_KEY before production startup.');
  }
}
...
export const env = {
  ...configuredEnv,
  MONGO_URI: configuredEnv.NODE_ENV === 'test' ? configuredEnv.MONGO_TEST_URI : configuredEnv.MONGO_URI,
};
