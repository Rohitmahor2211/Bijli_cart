...
const baseProductSchema = z.object({
  ...
  highlights: z.preprocess((value) => {
    if (typeof value !== 'string') return value;
    try { return JSON.parse(value); } catch { return value.split('\n'); }
  }, z.array(z.string().trim().min(1).max(160)).max(10).default([])),
  ...
  tax: z.coerce.number().min(0, 'GST/tax rate cannot be negative').max(100, 'GST/tax rate cannot exceed 100%').optional().default(0),
  stockQuantity: z.coerce.number().int().min(0).optional().default(0),
  lowStockThreshold: z.coerce.number().int().min(0).optional().default(5),
  weightKg: z.coerce.number().min(0.05, 'Package weight must be at least 0.05 kg').default(0.5),
  lengthCm: z.coerce.number().min(1, 'Package length is required').default(20),
  breadthCm: z.coerce.number().min(1, 'Package breadth is required').default(15),
...
const withProductRules = (schema) => schema.superRefine((product, context) => {
  if (product.mrp !== undefined && product.sellingPrice !== undefined && product.sellingPrice > product.mrp) {
    context.addIssue({ code: 'custom', path: ['sellingPrice'], message: 'Selling price cannot exceed MRP.' });
  }
});

export const createProductSchema = withProductRules(baseProductSchema);
export const updateProductSchema = withProductRules(baseProductSchema.partial());
...
export const productQuerySchema = z.object({
  ...
  sort: z.enum(['price_asc', 'price_desc', 'created_asc', 'created_desc', 'name_asc', 'name_desc']).optional(),
